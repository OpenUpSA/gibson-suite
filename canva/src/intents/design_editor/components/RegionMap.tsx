 ...
 * handles.
 */
export const RegionMap = ({ onRegionChange }: RegionMapProps) => {
  const intl = useIntl();
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: L.LeafletMouseEvent) => {
      ...
      const bounds = computeDragBounds(start, e.latlng, aspectRatioRef.current);
    ...
    const onMouseUp = (e: L.LeafletMouseEvent) => {
      ...
      if (Math.abs(maxLng - minLng) < 0.5 || Math.abs(maxLat - minLat) < 0.5) {
  ...
  const modeButtons: readonly {
    ...
    () => [
      ...
      { value: "full-globe", label: fullGlobeLabel, onClick: handleFullGlobe },
    ...
    <div className={styles.regionMapWrapper}>
      ...
      {mode === "specify" && (
        <div className={styles.regionMapCoords}>
          ...
          />
          <div>
            <Button variant="tertiary" size="small" onClick={handleApplyCoords}>
              {applyCoordsLabel}
            </Button>
          </div>
        </div>
